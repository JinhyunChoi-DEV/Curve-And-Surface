...
public class ShellLineRender : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        var maxDegree = data.MaxDegree;
        int maxCount = (maxDegree * (maxDegree + 1)) / 2;

        for (int i = 0; i < maxCount; ++i)
        {
            var point = Instantiate(pointPrafab, pointHolder.transform);
            point.gameObject.SetActive(false);
            shellPoints.Add(point);
        }

        for (int i = 1; i <= data.MaxDegree; ++i)
        {
            for (int j = 0; j < data.MaxDegree - i; ++j)
            {
                var newRender = Instantiate(linePrefab, lineHolder.transform);
                newRender.gameObject.SetActive(false);
                lineRender.Add(newRender);
            }
        }
    }
